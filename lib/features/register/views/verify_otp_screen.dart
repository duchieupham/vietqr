          ...
          return BlocConsumer<RegisterBloc, RegisterState>(
            ...
            listener: (context, state) {
              if (state.status == BlocStatus.SUCCESS &&
                  state.request == RegisterType.PHONE_AUTHENTICATION) {
                final type = state.typeOTP;
                if (type != null) {
                  widget.registerBloc.add(RegisterEventSentOTP(typeOTP: type));
                }
              }
              ...
              if (state.status == BlocStatus.UNLOADING &&
                  state.request == RegisterType.VERIFY_OTP) {
                if (!mounted) return;
                provider.updateLoading(false);
                Navigator.of(context).pop();
                widget.onChangePage(1);
              ...
              if (state.status == BlocStatus.ERROR &&
                  state.request == RegisterType.VERIFY_OTP) {
                provider.onOtpSubmit(state.msgVerifyOTP, () {
                  countdownProvider.setValue(0);
                });
              }
            },
                    ...
                    children: [
                                      ...
                                      recognizer: TapGestureRecognizer()
                                        ..onTap = () async {
                                          ...
                                          widget.registerBloc.add(
                                              RegisterEventPhoneAuthentication(
                                                  phone: widget.phone));
                      ...
                      ButtonWidget(
                        ...
                        function: () async {
                          ...
                          // }
                          widget.registerBloc.add(
                              RegisterEventVerifyOTP(otp: otpController.text));
                        },
                      ),
