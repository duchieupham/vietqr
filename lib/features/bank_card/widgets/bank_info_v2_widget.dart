...
class _BankInfroV2WidgetState extends State<BankInfroV2Widget>
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        children: [
          _buildInfoBank(widget.dto),
          if (!widget.dto.isAuthenticated)
            const SizedBox.shrink() //chua lien ket

          else if (inclusiveDays(widget.dto.validFeeTo) > 15 &&
              widget.dto.isAuthenticated) ...[ //con han
            const SizedBox(
              height: 15,
            ),
  ...
  InkWell _buildInfoService(BankAccountDTO dto) {
    return InkWell(
      onTap: () {
        showDialogActiveKey(
          context,
            ...
            children: [
              Row(
                ...
                children: [
                  ...
                  //sắp hết hạn
                  if (dto.isValidService &&
                      dto.validFeeTo != 0 &&
                      inclusiveDays(dto.validFeeTo) <= 15 &&
                      inclusiveDays(dto.validFeeTo) >= 0)
                    Padding(
                      padding: const EdgeInsets.only(right: 10),
                      child: Text(
                        inclusiveDays(dto.validFeeTo) == 0
              ...
              SizedBox(
                width: 345,
                child: Text(
                  (dto.validFeeTo == 0) // chua dang ky
                      ? contents[0]
                      : (dto.isValidService &&
                              dto.validFeeTo != 0 &&
                              inclusiveDays(dto.validFeeTo) <= 15 &&
                              inclusiveDays(dto.validFeeTo) > 0) // sap het han
                          ? contents[1]
                          : contents[2], // da qua han
  ...
  Widget _buildInfoBank(BankAccountDTO dto) {
    return Row(
      children: [
        Container(
          height: 30,
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(
              color:
                  const Color(0xFFE1EFFF), // Change this to your desired color
